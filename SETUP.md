...
3. Click **Run**. You should see "Success. No rows returned." This creates every table, sets up permissions, turns on real-time sync, seeds the four starter badges, and seeds an 8-step Focus scope & sequence for math, literacy, and personal finance (see `supabase/schema.sql`'s "Starter Focus scope & sequence" section, and `docs/PERSONAL_FINANCE_SCOPE.md` for the finance lane's own scaffolding notes — fully teacher-editable from Assignments → Focuses afterwards).
