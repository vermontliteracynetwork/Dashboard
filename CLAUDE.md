...
## Always deploy
Push every change to production. After any commit, push to `claude/new-session-nwv821` AND fast-forward-merge + push to `main` (Vercel auto-deploys `main`). Do this every time, with no exception, until the teacher says otherwise.

Merge sequence used throughout this project:
```
...
Production: https://independent-work-dashboard.vercel.app
