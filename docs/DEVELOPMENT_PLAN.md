...
- **Build Mode punch-list** (one detailed teacher message; click-drag rotate SHIPPED this hour, rest still open): wall pieces placed adjacent to each other don't join into a clean corner/intersection (confirmed — no wall-corner-joining logic exists in `WorldEditor.tsx`); door/window placement directly onto a wall surface; showing only a selected asset's exterior (not interior) when viewing it from outside a building; smoothing the movement-collision glitch near placed objects (should ease the player around/stop cleanly, not visibly glitch) — the underlying "don't walk through it" collision itself is shipped (task #92), only the glitch-smoothing polish is open; and making the student-facing Build Mode's own catalog view mirror the teacher catalog's layout (filtered to what that student actually owns) with a marketplace-purchase indicator for locked items they could buy. Direct teacher framing: "Sims 4 is my favorite building platform and i want the controls for mac to be the model for our build mode."
