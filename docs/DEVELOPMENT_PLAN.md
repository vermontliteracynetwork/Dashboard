...
- **Computer UI — card/widget-based dashboard redesign.** Direct teacher ask (with 2 reference images): student computers should show "a frame, and a simple background... lots of images, card based design, dashboard style," with a card for email (shared with the mailbox inbox) and a card for online banking. A separate, later message asked for this to go further: "each thing should look like widgets on a Mac. A real calendar should be visual as a widget, an analog clock... the visual of the to do list with live links... bank in an app view," eventually with drag-to-rearrange and theming. Confirmed NOT built — `StudentHome.tsx`'s computer entry point still uses the simpler retro-early-2000s-browser-frame treatment (A-series, task history #70), not this widget/card dashboard. That frame is a real, separate, already-shipped feature and isn't wrong to keep; this item is the further redesign on top of it.
