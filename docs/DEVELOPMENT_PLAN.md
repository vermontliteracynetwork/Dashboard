...
- **"Show all home assets with Claudia's size/cost reference" backlog item — PARKED, needs a teacher pricing decision, not built.** Investigated this hour: Build Mode's object catalog (the full `asset-manifest.json` set, thousands of items across dozens of categories) is entirely free to place today — `WorldEditor.tsx` has no price/currency check anywhere in it. The Marketplace (`marketplaceSeed.ts`/`Marketplace.tsx`) is a completely separate system selling only fonts/colors/voices/power-ups/emotes, none of it Build Mode content. `docs/SIZE_REFERENCE.md` (checked in full) defines size *units* only — there is no existing "cost reference" table to pull real prices from; one would have to be invented from scratch. Building this for real means a real economy decision only the teacher can make: does placing home/build items stay free forever (Marketplace just adds an optional priced subset alongside free Build Mode), or does some/all of Build Mode become a real purchase gate (a much bigger behavior change for something currently unlimited)? Needs the teacher's call on scope and actual price points before this is buildable.
...
- **Full sky texture — SHIPPED this hour (fifth attempt), needs the teacher's own visual check before it's trusted.** Four prior attempts at a photographic/equirectangular sky (`SkyboxBackground` in `TownSquare.tsx`) all broke visibly on the horizon once actually seen live (jagged translucent blue wedge artifacts), and the code's own standing comment says not to retry that technique without a real live visual check. This hour swapped in drei's procedural `<Sky>` (a real atmospheric-scattering shader, no image or UV mapping at all, so it cannot produce the same pole-seam artifact class by construction) as the DEFAULT look, only when the teacher hasn't picked a sky tint — picking a tint (paint bucket) still falls back to the same flat-color approach that's always worked, unchanged. Also fixed a related pre-existing inconsistency while touching this code: `SkyboxBackground` previously ignored the teacher's chosen `skyColor` entirely and always showed flat `#bfe3ff`, unlike `WorldEditor.tsx`'s own live preview which already respects it — Live Mode now matches Build Mode. **Could not live-screenshot-verify in this sandboxed session**: the app requires real Supabase credentials to render past its connection-check screen, and none are configured here (by design, this is a dev sandbox, not the teacher's live backend), so there was no way to actually reach Town Square and look at it, the same fundamental limitation the original comment already names. Shipped anyway based on the shader being structurally incapable of the specific failure class, and because this only reaches `claude/new-session-nwv821` (per the current no-main-push policy), never production, until the teacher reviews it herself — but this is genuinely the first thing to look at before trusting it, given the history.
...
- **Computer UI — card/widget-based dashboard redesign — SHIPPED this hour (v1), drag-rearrange + theming still parked.** `StudentHome.tsx` still opens inside the retro browser-frame chrome (A-series, task #70, a real separate feature, kept as-is), but the body below it is now a real Mac-desktop-style `.desktop-widget-grid` of icon/widget cards instead of a row of small text buttons: a brand-new live analog clock widget (`AnalogClock`, SVG hands driven off the screen's existing 1s tick, no second interval), a brand-new calendar widget (month/day-number/weekday, styled like a torn-page desk calendar), and icon cards for Mail (now links the already-built but previously home-disconnected `Mailbox.tsx`, with a real unread-style count badge off `worldQuest1MetIds`), Bank (balance, links Piggy Bank), Marketplace, Achievements (badge count), Daily Spin (still outlined when not yet spun today), Chat, What's New, and Town Square. The Math/Literacy subject tiles (the actual "to-do list with live links") were left exactly as they already were — already a card-style launcher, not a plain list, so reusing rather than duplicating them. **Not built:** drag-to-rearrange and per-student theming from the same original ask — both need new persisted state (a widget layout array / theme choice on `Student`, synced like every other field) rather than a UI-only change, a real next phase, not a same-hour add-on.
...
- **Farmer's Market — barter/negotiation mechanic.** A separate marketplace location (distinct from the regular Store) appearing at certain times with unique/seasonal items, where students trade items of similar value with each other rather than paying coins — explicitly framed by the teacher as a way to teach negotiation as a real skill. Confirmed not built (no `farmer`/barter code anywhere in `src/`).
- **Playground — meme/personality-quiz/mini-game gallery — image half SHIPPED this hour, quiz + mini-games still parked.** From an early Playground-overhaul brainstorm, referencing the Kinzoo messaging app's kid-facing marketplace as a model: "a gallery where i can add videos and images for the kids to look through... fun things like memes, mini games, and silly personality quizzes." Built this hour: a new `GalleryItem` type/table (`gallery_items`, permalink `https://github.com/vermontliteracynetwork/Dashboard/blob/9b4505d/supabase/schema.sql#L254-L260`), a teacher `GalleryManager.tsx` (upload/tag/caption/delete images, same pattern as `CinemaVideosManager.tsx`), and a student-facing `Gallery.tsx` (tap-to-enlarge browsing grid, reached from a new tile in the Playground, no scoring/completion state, matching this doc's "no explicit activity" rule). Deliberately scoped to images only — the video half of the original ask isn't duplicated since `Cinema.tsx` already covers "watch for fun, unlimited replay, teacher-curated" exactly. **Still not built:** personality quizzes (a real ABA/SEL tone question for this population — self-report "which type are you" categorization can read as comparative or anxiety-inducing for autistic/ADHD students, so this needs a Claudia design pass before it's built, not a same-hour blind build) and mini games (distinct from the native games already shipped; no scope has been written for what a "mini game" here would actually be, a real design question, not implementation).
