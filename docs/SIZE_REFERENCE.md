...
**`HomeRoom.tsx`** (a student's own furniture, placed in their private room) uses a **third, different** sizing convention — items are fit to a footprint width or bounding-cube diagonal, never to `CHARACTER_HEIGHT`. **Correction (Claudia's audit found this is not an oversight):** the couch/bed/cabinet footprint/cube targets are a direct, verbatim teacher instruction ("a couch should be about two square units long and one square unit high... kitchen cabinets should be one square unit high and tall and wide, just a square" — see `HomeRoom.tsx` lines 28-34) — a protected design decision, not a bug to silently override with `CHARACTER_HEIGHT` logic. The real gap: `computeStarterScale` only produces one uniform scalar (matching either the length target OR the height target, never both at once for the same item), so it can't actually hit both of the teacher's own targets simultaneously the way she specified. Fixing that properly needs per-axis (x/y/z) scale storage, but `WorldObject.scale` is a single `number` applied uniformly everywhere (`types.ts`, `WorldObjectRenderer.tsx`) shared across Town Square/Island/HomeRoom/sync — a real schema change touching every placed-object surface in the app, not a small fix. The genuinely unreconciled "1.0 = person height" gap is narrower than this section previously implied: it's really only the door/window class of items that should read ~1.0x person height and currently don't, not the couch/bed/cabinet sizing, which is working as specified.
