...
-- ---------------------------------------------------------------------------
-- Starter Focus, plus its documented scope & sequence (teacher-published
-- from Assignments → Focuses when the class is ready to move on) — Common
-- Core aligned, ordered to match the real skill progression in UFLI
-- Foundations (literacy) and The Math Learning Center's Bridges
-- Intervention volumes (math). Claudia's review: auto-scheduling all 8
-- steps on a fixed weekly wall-clock timer risked the ambient dialogue
-- reinforcement (see lib/focus.ts's maybeAppendFocusLine) drifting ahead of
-- whatever pace the teacher's own explicit instruction is actually on — so
-- only the FIRST step of each sequence is seeded as active (untilChanged,
-- no end date); she publishes the next one herself when the class has
-- actually moved on, using the same Focus editor already built for this.
--
-- Full sequence, for reference when publishing the next step:
--   Literacy: Alphabet Code -> Short Vowels -> Digraphs (sh, ch, th, wh) ->
--     Silent-E Pattern -> R-Controlled Vowels -> Vowel Teams -> Diphthongs
--     -> Prefixes & Suffixes
